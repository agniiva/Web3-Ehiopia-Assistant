...
def page_config():
    st.set_page_config(page_title="Web3 Training Ehiopia", page_icon="computer")
...
def page_title_header():
    ...
    st.image(top_image)
    st.title("Coding Assistant for Web3 Training Ehiopia")
    st.caption("Partnered with Invest for Jobs, Sequa, A|Q Forensics, IceAddis, Eaglelion")
...
def sidebar():
    ...
    languages = sorted(['Solidity','Python', 'GoLang','TypeScript', 'JavaScript', 'Java', 'C', 'C++', 'C#', 'R', 'SQL'])
        ...
        if st.session_state.user_authenticated:
            with st.expander(label="Settings",expanded=True):
                ...
                with chatbot_settings_tab:
                    model = st.selectbox("Language Model", options=['gpt-4-0613','gpt-4-1106-preview','gpt-3.5-turbo-1106'])
...
def display_all_code():
    ...
    st.write("https://github.com/agniiva/Web3-Ehiopia-Assistant")
